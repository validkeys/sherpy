...
import type { Project, Document, ChatMessageResponse } from '@sherpy/shared';
...
class BackendValidator {
  ...
  private async validateApiEndpoints(): Promise<void> {
    ...
    if ((this as any).testProjectId) {
      await this.checkEndpoint(
        ...
        async () => {
          const projectId = (this as any).testProjectId;
          const response = await this.authenticatedFetch(`${this.apiUrl}/projects/${projectId}/documents`);
          const data = await response.json();
          return {
  ...
  private async validateDatabaseSchema(): Promise<void> {
    ...
    await this.checkEndpoint(
      'Database: projects.pipelineStatus field',
      async () => {
        const projectId = (this as any).testProjectId;
        if (!projectId) {
          return {
    ...
    await this.checkEndpoint(
      'Database: documents table structure',
      async () => {
        const projectId = (this as any).testProjectId;
        if (!projectId) {
          return {
  ...
  private async validateSkillInvocation(): Promise<void> {
    await this.checkEndpoint(
      'Skills: Programmatic invocation capability',
      async () => {
    ...
    await this.checkEndpoint(
      'Skills: Document generation integration',
      async () => {
        const projectId = (this as any).testProjectId;
        if (!projectId) {
          return {
    ...
    await this.checkEndpoint(
      'Skills: Completion webhooks/callbacks',
      async () => {
  ...
  private async checkEndpoint(
    ...
    try {
      ...
      const color = fullResult.status === 'green' ? '\x1b[32m' : fullResult.status === 'yellow' ? '\x1b[33m' : '\x1b[31m';
...
async function main() {
  ...
  if (report.blockers > 0) {
    ...
    report.results.filter(r => r.blocker).forEach(r => {
...
async function writeReport(report: ValidationReport, path: string): Promise<void> {
  ...
  if (report.confidenceLevel === 'high') {
    ...
    report.results.filter(r => r.status === 'yellow').forEach(r => {
      md.push(`- ${r.name}: ${r.details}`);
    });
  } else {
