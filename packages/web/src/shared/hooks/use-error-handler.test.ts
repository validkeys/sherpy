...
describe('useErrorHandler', () => {
  ...
  describe('retry', () => {
    ...
    it('throws after max retries exceeded', async () => {
      ...
      await expect(
        result.current.retry(mockFn, { maxRetries: 2, delay: 10 }),
    ...
    it('does not retry non-retryable errors', async () => {
      ...
      await expect(
        result.current.retry(mockFn, { maxRetries: 3, delay: 10 }),
    ...
    it('applies exponential backoff delay', async () => {
      ...
      await expect(
        result.current.retry(mockFn, { maxRetries: 3, delay: 50 }),
  ...
  describe('toast integration', () => {
    it('shows toast when enabled and provided', () => {
      ...
      const { result } = renderHook(() =>
        useErrorHandler({ showToast: true, toast: mockToast }),
      );
    ...
    it('does not show toast when disabled', () => {
      const mockToast = vi.fn();
      const { result } = renderHook(() =>
        useErrorHandler({ showToast: false, toast: mockToast }),
      );
