...
import { OktaClaims, validateJwt } from "./okta-jwt.js";
  ...
  validateToken: (token: string) =>
    Effect.gen(function* () {
      ...
      if (token === expiredToken) {
        return yield* Effect.fail(new UnauthorizedError({ message: "Token expired" }));
      }
...
describe("AuthService - JWT Signature Verification", () => {
  ...
  it.effect("rejects tokens with invalid signatures", () =>
    Effect.gen(function* () {
      const authService = yield* AuthService;
      const result = yield* Effect.either(authService.validateToken("invalid.signature.token"));
