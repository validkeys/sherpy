...
import { AuthService, OktaClaims } from "./jwks-cache.js"
import { UnauthorizedError } from "../errors/auth.js"

/**
 * Validate JWT token and extract claims
 */
export const validateJwt = (token: string): Effect.Effect<OktaClaims, UnauthorizedError, AuthService> =>
  Effect.gen(function* () {
    if (!token) {
      return yield* Effect.fail(
        new UnauthorizedError({ message: "No token provided" })
      )
    }
