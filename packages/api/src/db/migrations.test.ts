...
describe("Database Migrations", () => {
  describe("Migration Execution", () => {
    it.scoped("runs all migrations successfully", () =>
      Effect.gen(function* () {
    ...
    it.scoped("is idempotent - can run migrations multiple times", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
  ...
  describe("Migration 001: Core Tables", () => {
    it.scoped("creates projects table with correct schema", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates projects table indexes", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates milestones table with foreign key", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates milestones table indexes", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates tasks table with foreign keys", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
        ...
        expect(fks.some((fk) => fk.from === "milestone_id" && fk.table === "milestones")).toBe(true)
        expect(fks.some((fk) => fk.from === "project_id" && fk.table === "projects")).toBe(true)
      }) as Effect.Effect<void>,
    )

    it.scoped("creates tasks table indexes", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates tags table with unique constraint", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
  ...
  describe("Migration 002: People and Resource Tables", () => {
    it.scoped("creates people table with unique email", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates skills table", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates person_skills junction table with composite primary key", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates assignments table", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates availability_windows table", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
  ...
  describe("Migration 003: Documents, Chat, and Schedules Tables", () => {
    it.scoped("creates documents table with unique constraint", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates chat_sessions table", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("creates schedule_snapshots table", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
  ...
  describe("Referential Integrity", () => {
    it.scoped("enforces CASCADE delete for project-milestone relationship", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
    ...
    it.scoped("enforces CASCADE delete for milestone-task relationship", () =>
      Effect.gen(function* () {
        const sql = yield* makeTestClient
        yield* runMigrations.pipe(
          Effect.provideService(SqlClient.SqlClient, sql),
