 ...
 */
export class UpdateTagInput extends Schema.Class<UpdateTagInput>(
  "UpdateTagInput",
)({
 ...
 */
export class TagService extends Effect.Service<TagService>()(
  "TagService",
  {
    effect: Effect.gen(function* () {
      ...
      const get = (
        id: string,
      ): Effect.Effect<typeof Tag.Type, NotFoundError | ValidationError> =>
        Effect.gen(function* () {
          const tags = yield* sql<typeof Tag.Type>`
            SELECT * FROM tags WHERE id = ${id}
          `
      ...
      const remove = (
        ...
        Effect.gen(function* () {
          ...
          for (const project of projects) {
            const tagIds = typeof project.tags === 'string'
      ...
      const assignToProject = (
        ...
        Effect.gen(function* () {
          ...
          if (!Array.isArray(tagIds) || !tagIds.includes(tagId)) {
            const updatedTags = Array.isArray(tagIds)
              ? [...tagIds, tagId]
              : [tagId]
      ...
      const getProjectTags = (
        ...
        Effect.gen(function* () {
          ...
          const tags = yield* Effect.all(
            tagIds.map((tagId: string) => get(tagId)),
          )
