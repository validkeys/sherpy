 ...
 */
export class ReorderTasksInput extends Schema.Class<ReorderTasksInput>("ReorderTasksInput")({
 ...
 */
export class ListTaskFilters extends Schema.Class<ListTaskFilters>("ListTaskFilters")({
...
export class TaskService extends Effect.Service<TaskService>()("TaskService", {
  effect: Effect.gen(function* () {
     ...
     * Get a task by ID
     */
    const get = (id: string): Effect.Effect<typeof Task.Type, NotFoundError | ValidationError> =>
      Effect.gen(function* () {
        const tasks = yield* sql<typeof Task.Type>`
            SELECT * FROM tasks WHERE id = ${id}
          `;
    ...
    const bulkUpdateStatus = (
      ...
      Effect.gen(function* () {
        ...
        // Fetch updated tasks
        const updatedTasks = yield* Effect.all(input.taskIds.map((id) => get(id)));
