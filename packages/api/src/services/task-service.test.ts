...
import { Task, NotFoundError, ValidationError } from "@sherpy/shared"
...
describe("TaskService", () => {
  it.scoped("create - creates task with automatic orderIndex", () =>
    Effect.gen(function* () {
      ...
      const projectService = yield* ProjectService.pipe(
        Effect.provide(ProjectServiceLive.pipe(
          Layer.provide(Layer.succeed(SqlClient.SqlClient, sql))
        ))
      )

      const milestoneService = yield* MilestoneService.pipe(
        Effect.provide(MilestoneServiceLive.pipe(
          Layer.provide(Layer.succeed(SqlClient.SqlClient, sql))
        ))
      )

      const service = yield* TaskService.pipe(
        Effect.provide(TaskServiceLive.pipe(
          Layer.provide(Layer.succeed(SqlClient.SqlClient, sql))
        ))
      )
      ...
      const milestone = yield* milestoneService.create({ projectId: project.id, name: "Test Milestone" })
  ...
  it.scoped("create - fails when milestone does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create - fails when milestone does not belong to project", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const milestone = yield* milestoneService.create({ projectId: project1.id, name: "Milestone 1" })
  ...
  it.scoped("create - sets orderIndex sequentially within milestone", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const t1 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 1" })
      const t2 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 2" })
      const t3 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 3" })
  ...
  it.scoped("listByMilestone - returns tasks ordered by orderIndex", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("listByProject - returns tasks with optional status filter", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const t1 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 1", priority: "high" })
      const t2 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 2", priority: "low" })
      const t3 = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Task 3", priority: "medium" })
  ...
  it.scoped("get - returns task by ID", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const project = yield* projectService.create({ name: "Test Project" })
      const milestone = yield* milestoneService.create({ projectId: project.id, name: "Test Milestone" })
      const created = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Test Task" })
  ...
  it.scoped("get - fails when task does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("update - updates task fields", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const project = yield* projectService.create({ name: "Test Project" })
      const milestone = yield* milestoneService.create({ projectId: project.id, name: "Test Milestone" })
      const created = yield* service.create({ milestoneId: milestone.id, projectId: project.id, name: "Original Name", priority: "low" })
  ...
  it.scoped("reorder - reorders tasks within milestone atomically", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("reorder - fails when task does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("reorder - fails when not all tasks are included", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("bulkUpdateStatus - updates multiple tasks atomically", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("bulkUpdateStatus - fails when task does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
