...
import { Project, NotFoundError, ConflictError } from "@sherpy/shared"
...
describe("ProjectService", () => {
  it.scoped("create - creates project with generated slug", () =>
    Effect.gen(function* () {
  ...
  it.scoped("create - creates project with provided slug", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("list - returns all projects", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("get - returns project by ID", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("update - updates project name", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
