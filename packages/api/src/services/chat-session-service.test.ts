...
describe("ChatSessionService", () => {
  describe("create", () => {
    it.scoped(
      "creates a new chat session with empty messages",
      () =>
        Effect.gen(function* () {
          ...
          const allServices = Layer.mergeAll(ProjectServiceLive, ChatSessionServiceLive).pipe(
  ...
  describe("addMessage", () => {
    it.scoped(
      "appends a message to an empty session",
      () =>
        Effect.gen(function* () {
          const sql = yield* makeTestDb;
          yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
    ...
    it.scoped(
      "fails when session does not exist",
      () =>
        Effect.gen(function* () {
          const sql = yield* makeTestDb;
          yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  describe("getHistory", () => {
    ...
    it.scoped(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const result = yield* chatService.getHistory("non-existent-session").pipe(Effect.flip);
    ...
    it.scoped(
      "returns empty messages array for new session",
      () =>
        Effect.gen(function* () {
          const sql = yield* makeTestDb;
          yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  describe("listByProject", () => {
    ...
    it.scoped(
      "does not return sessions from other projects",
      () =>
        Effect.gen(function* () {
          const sql = yield* makeTestDb;
          yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
    ...
    it.scoped(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const result = yield* chatService.delete("non-existent-session").pipe(Effect.flip);
