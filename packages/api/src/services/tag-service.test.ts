...
import { Tag, ConflictError, NotFoundError, ValidationError } from "@sherpy/shared"
...
describe("TagService", () => {
  it.scoped("create - creates tag with name and color", () =>
    Effect.gen(function* () {
      ...
      const service = yield* TagService.pipe(
        Effect.provide(TagServiceLive.pipe(
  ...
  it.scoped("create - creates tag without color", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create - fails when tag name already exists", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const result = yield* Effect.either(
        service.create({ name: "Frontend", color: "#00ff00" })
      )
  ...
  it.scoped("list - returns all tags ordered by name", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("get - retrieves tag by id", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("get - fails when tag does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("update - updates tag name", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("update - updates tag color", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("update - fails when new name conflicts with existing tag", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const result = yield* Effect.either(
        service.update(tag2.id, { name: "Frontend" })
      )
  ...
  it.scoped("remove - removes tag from all projects", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const tags1 = typeof updated1.tags === 'string'
        ? JSON.parse(updated1.tags)
        : updated1.tags
      const tags2 = typeof updated2.tags === 'string'
        ? JSON.parse(updated2.tags)
        : updated2.tags
  ...
  it.scoped("assignToProject - adds tag to project", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
      ...
      const tagIds = typeof updated.tags === 'string'
  ...
  it.scoped("assignToProject - is idempotent", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("assignToProject - fails when tag does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("assignToProject - fails when project does not exist", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("removeFromProject - removes tag from project", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("getProjectTags - returns all tags for a project", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("getProjectTags - returns empty array when project has no tags", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
