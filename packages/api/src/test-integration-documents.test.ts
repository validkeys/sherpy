...
describe("Document Generation Integration Tests", () => {
  it.scoped(
    "generate YAML document → verify valid YAML structure",
    () =>
      Effect.gen(function* () {
  ...
  it.scoped(
    "generate markdown document → verify markdown format",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "generate JSON document → verify valid JSON structure",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "version history: generate → modify → generate → verify versions",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "get latest document retrieves most recent version",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "document reflects current DB state with tasks and milestones",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "empty project generates valid document",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
  ...
  it.scoped(
    "document preserves special characters and newlines in descriptions",
    () =>
      Effect.gen(function* () {
        const sql = yield* makeTestDb;
        yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql));
