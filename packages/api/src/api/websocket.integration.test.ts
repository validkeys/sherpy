...
describe("WebSocket Integration Tests", () => {
    ...
    wss.on("connection", (ws: WebSocket, request) => {
      ...
      Effect.runPromise(
        Effect.gen(function* () {
          ...
          ws.on("close", () => {
            Effect.runPromise(broadcaster.removeConnection(connection).pipe(Effect.orDie));
          });
        }),
      );
    });
  ...
  it("should accept WebSocket connection with valid JWT", async () => {
    const client = new WebSocket(`ws://127.0.0.1:${serverPort}?token=valid-token-123`);
  ...
  it("should reject WebSocket connection with invalid JWT", async () => {
    const client = new WebSocket(`ws://127.0.0.1:${serverPort}?token=invalid-token`);
    testClients.push(client);
  ...
  it("should broadcast event to all connected clients", async () => {
    // Connect two clients
    const client1 = new WebSocket(`ws://127.0.0.1:${serverPort}?token=valid-token-123`);
    const client2 = new WebSocket(`ws://127.0.0.1:${serverPort}?token=valid-token-123`);
