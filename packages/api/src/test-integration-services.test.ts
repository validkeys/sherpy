...
import { NotFoundError, ConflictError } from "@sherpy/shared"
...
describe("Service Integration Tests", () => {
  it.scoped("create project → create milestone → create tasks → verify ordering", () =>
    Effect.gen(function* () {
  ...
  it.scoped("create tags → assign to project → get project tags → verify", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create milestone → reorder → verify order persisted", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create tasks → bulk status update → verify all updated", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create project → create chat session → add messages → retrieve history", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("create tag → delete tag → verify removed", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("error: create milestone with non-existent project → NotFoundError", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("error: create task with non-existent milestone → NotFoundError", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("error: create duplicate tag name → ConflictError", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("error: update non-existent project → NotFoundError", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("list chat sessions for project ordered by most recent", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("task reordering within milestone", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
  ...
  it.scoped("list tasks by project with status filter", () =>
    Effect.gen(function* () {
      const sql = yield* makeTestDb
      yield* runMigrations.pipe(Effect.provideService(SqlClient.SqlClient, sql))
